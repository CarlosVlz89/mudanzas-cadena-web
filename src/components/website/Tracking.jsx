...
const Tracking = () => {
  ...
  // --- BASE DE DATOS FALSA (MOCK DATA) PARA PRUEBAS ---
  // Esto simula lo que luego vendrá de Firebase
  const mockDatabase = {
    'MUD-123': {
      cliente: 'Familia González',
      origen: 'CDMX, Coyoacán',
      destino: 'Mérida, Yucatán',
      status: 'En Tránsito', // Opciones: Recibido, En Carga, En Tránsito, Finalizado
      fecha: '20 de Enero, 2026',
      ubicacionActual: 'Puebla, Esperanza (Autopista 150D)',
      porcentaje: 60
    },
    'MUD-456': {
      ...
      ubicacionActual: 'Entregado en Bodega Norte',
      porcentaje: 100
    },
    'MUD-789': {
      cliente: 'Juan Pérez',
      origen: 'Ecatepec, Edo Mex',
      destino: 'Cancún, Quintana Roo',
      status: 'En Carga',
      fecha: '21 de Enero, 2026',
      ubicacionActual: 'Bodega Mudanzas Cadena',
      porcentaje: 25
    }
  };
  ...
  const handleSearch = (e) => {
    ...
    // Simulamos un retraso de internet (1.5 segundos) para dar realismo
    setTimeout(() => {
      const data = mockDatabase[folio.toUpperCase()]; // Convertimos a mayúsculas para evitar errores
      
      if (data) {
        setResult(data);
      } else {
        setError('No encontramos ninguna mudanza con ese folio. Verifica que esté bien escrito (ej: MUD-123).');
      }
      setLoading(false);
    }, 1500);
  };

  // Función para pintar el color de la barra según el estado
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-3xl mx-auto text-center mb-12">
        ...
        <p className="text-gray-600 mb-8">
          Ingresa tu número de folio para conocer el estado actual de tu servicio en tiempo real.
        </p>
        ...
        <form onSubmit={handleSearch} className="relative max-w-md mx-auto">
          <input
            type="text"
            placeholder="Ejemplo: MUD-123"
            value={folio}
            onChange={(e) => setFolio(e.target.value)}
            className="w-full px-6 py-4 rounded-full border-2 border-gray-200 focus:border-cadena-blue focus:ring-4 focus:ring-cadena-blue/20 outline-none transition text-lg shadow-sm"
          />
          <button 
            type="submit"
            disabled={loading}
      ...
      {/* 2. TARJETA DE RESULTADOS (Solo aparece si hay resultado) */}
      {result && (
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 animate-fade-in-up">
          ...
          <div className="bg-cadena-dark text-white p-6 flex justify-between items-center flex-wrap gap-4">
            <div>
              ...
              <h3 className="text-xl font-bold">{result.cliente}</h3>
            </div>
            <div className="text-right">
              ...
              <span className={`inline-block px-3 py-1 rounded-full text-sm font-bold mt-1 ${
                result.status === 'Finalizado' ? 'bg-green-500 text-white' : 'bg-cadena-pink text-white'
              }`}>
                {result.status}
              </span>
            </div>
          ...
          <div className="p-8">
            ...
            <div className="flex items-center justify-between mb-8 text-cadena-dark">
              <div className="text-center w-1/3">
                ...
                <p className="font-bold text-lg">{result.origen}</p>
                <p className="text-xs text-gray-500">Origen</p>
              </div>
              <div className="flex-1 flex flex-col items-center px-4">
                <ArrowRight className="text-cadena-blue w-full h-6" />
                <p className="text-xs text-gray-400 mt-1">{result.fecha}</p>
              </div>
              <div className="text-center w-1/3">
                <MapPin className="mx-auto text-cadena-pink mb-2" />
                <p className="font-bold text-lg">{result.destino}</p>
            ...
            <div className="mb-8">
              ...
              <p className="mt-3 text-center text-sm font-medium text-cadena-blue bg-blue-50 py-2 rounded-lg">
                📍 Ubicación reporte: {result.ubicacionActual}
              </p>
            </div>
...
// Componente pequeño auxiliar para los pasos
