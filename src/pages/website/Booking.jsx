...
import { Truck, CheckCircle, Plus, Minus, Sofa, Tv, Box, Bed, WashingMachine, Refrigerator, ArrowLeft } from 'lucide-react';
            ...
            {step === 2 && (
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase ml-4">¿Desde dónde?</label>
                    <input required placeholder="Ciudad / Colonia" className="w-full p-4 bg-white/50 rounded-2xl border-2 border-white focus:border-cadena-blue outline-none shadow-sm" 
                      value={form.origin} onChange={e => setForm({...form, origin: e.target.value})} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase ml-4">¿Hacia dónde?</label>
                    <input required placeholder="Ciudad / Colonia" className="w-full p-4 bg-white/50 rounded-2xl border-2 border-white focus:border-cadena-blue outline-none shadow-sm" 
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {FURNITURE_ITEMS.map((item) => (
                      <div key={item.id} className={`p-4 rounded-[2rem] border-2 flex flex-col items-center justify-between transition-all duration-300 ${itemsCount[item.id] > 0 ? 'border-cadena-blue bg-white shadow-lg shadow-blue-100 scale-105' : 'border-white bg-white/30 opacity-70'}`}>
                        <div className={`mb-2 ${itemsCount[item.id] > 0 ? 'text-cadena-blue' : 'text-gray-400'}`}>{item.icon}</div>
                        <span className="text-[10px] font-black text-center mb-3 leading-tight uppercase tracking-tighter text-gray-700">{item.label}</span>
                        
                        <div className="flex items-center gap-3 bg-slate-100 rounded-2xl px-3 py-2">
                          <button type="button" onClick={() => updateCount(item.id, -1)} className="text-gray-400 hover:text-red-500 transition"><Minus size={14} /></button>
                          <span className="font-black w-4 text-center text-xs text-cadena-dark">{itemsCount[item.id] || 0}</span>
                          <button type="button" onClick={() => updateCount(item.id, 1)} className="text-cadena-blue hover:text-blue-700 transition"><Plus size={14} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
